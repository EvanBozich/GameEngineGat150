...
namespace nu
{
	...
	void RigidBodyPhysicsComponent::SetRotaion(float roation)
	{
		GetOwner()->SetRoation(roation);
	}

	float RigidBodyPhysicsComponent::GetRotaion() const
	{
	...
	void RigidBodyPhysicsComponent::Read(const json::value_t& value)
	{
		...
		JSON_READ_NAME(value, "anglar_velocity", m_angularVelocity);
