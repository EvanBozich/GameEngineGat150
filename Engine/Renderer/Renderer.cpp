...
namespace nu 
{
    ...
    void Renderer::SetColorInt(Uint8 r, Uint8 g, Uint8 b, Uint8 a) const
    {
    ...
    void Renderer::DrawTexture(const Texture& texture, float x, float y, float angle, float scale, bool flipH, const Vector2& origin) const
    {
        ...
        float cameraX = (m_cameraEnabled) ? (m_camera.x - 640) : 0.0f;
        float cameraY = (m_cameraEnabled) ? (m_camera.y - 512) : 0.0f;
